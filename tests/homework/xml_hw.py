...
def find_top3_poorest_city() -> list:
    ...
    for xml_data in list_xml_data:
        if xml_data['city'] in dict_city:
            dict_city[xml_data['city']][0] += int(xml_data['est_asset_dollar']) # sum(est_asset_dollar)
            dict_city[xml_data['city']][1] += 1 # count
        else:
            dict_city[xml_data['city']] = [int(xml_data['est_asset_dollar']), 1]
